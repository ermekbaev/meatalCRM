...
import { MATERIALS, DENSITY_OPTIONS, HISTORY_KEY } from "./_constants";
import type { HistoryEntry, CalcMode } from "./_types";
import { formatNum, formatCur } from "./_utils";
import { ResultRow } from "./ResultRow";

// Извлечено из app/(dashboard)/calculator/page.tsx (п.8 REMEDIATION —
// разбивка god-компонента, логика не менялась). Тело присоединяется ниже.
export function MetalCalculator({ mode }: { mode: CalcMode }) {
  const initThicknesses = useMemo(
    () => getThicknessesForMaterial("hot-rolled"),
    [],
  );
  const initThickness = initThicknesses[0] ?? 0;
  const initSizes = useMemo(
    () => getSizesForMaterial("hot-rolled", initThickness),
    [initThickness],
  );
  ...
  const [clientResults, setClientResults] = useState<
    { id: string; name: string; discount?: number | null }[]
  >([]);
  ...
  function calculate() {
    ...
    const clientDiscount =
      mode === "counterparty" && selectedClient?.discount
                    ...
                    >
                      ...
                      >
                        <p className="text-xs font-medium text-slate-700 truncate">
                          {h.materialLabel} · {h.thickness} мм · {h.width}×
                          {h.length}
                        </p>
                        <p className="text-xs text-slate-400 mt-0.5">
                          {h.quantity} шт · {h.totalMass.toFixed(1)} кг ·{" "}
                          {h.costWithMarkup.toLocaleString("ru-RU")} ₽
                        </p>
                        <p className="text-[10px] text-slate-300 mt-0.5">
                          {new Date(h.ts).toLocaleString("ru-RU", {
                            day: "2-digit",
                            month: "2-digit",
                            hour: "2-digit",
                            minute: "2-digit",
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                {selectedClient ? (
                    ...
                    <div>
                      ...
                      {selectedClient.discount ? (
                        <p className="text-xs text-blue-500">
                          Скидка: {selectedClient.discount}% (применена
                          автоматически)
                        </p>
                      ) : null}
                    </div>
                  ...
                  <div className="relative">
                    ...
                    />
                    {clientLoading && (
                      <p className="absolute right-3 top-2.5 text-xs text-slate-400">
                        Поиск...
                      </p>
                    )}
            ...
            {weightMode === "invoice" && (
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  value={invoiceMass}
                  onChange={(e) => {
                    setInvoiceMass(e.target.value);
                    setInvoiceMassEdited(true);
                    setResult(null);
                  }}
                ...
                {!refEntry && thickness > 0 && (
                  <p className="text-[11px] text-slate-400">
                    Размер не найден в справочнике — введите фактический вес
                  </p>
                )}
              </div>
            )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    className={
                      !useCustomSize ? "bg-slate-50 text-slate-500" : ""
                    }
                  />
                </div>
                <div className="space-y-1">
            ...
            <div className="flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2.5">
              <div>
                <p className="text-sm font-medium text-slate-700">НДС 22%</p>
                <p className="text-[11px] text-slate-400">
                  Добавить к итоговой стоимости
                </p>
              </div>
              <Switch
                id="vat"
            ...
            {!result ? (
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-xl bg-slate-50 border border-slate-200 p-4 space-y-3">
                  ...
                  <ResultRow
                    label="Площадь листа"
                    value={`${formatNum(result.area)} м²`}
                  />
                  <ResultRow
                    label="Масса 1 листа"
                    value={`${formatNum(result.massPerSheet)} кг`}
                    highlight
                  />
                  {result.refMass !== undefined &&
                    result.refMass !== result.massPerSheet && (
                      <ResultRow
                        label="По справочнику"
                        value={`${formatNum(result.refMass)} кг`}
                      />
                    )}
                  <ResultRow
                    label="Удельный вес"
                    value={`${formatNum(result.massPerSqM)} кг/м²`}
                  />
                  <ResultRow
                  ...
                  {result.vatAmount > 0 && (
                    <>
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-sm text-slate-300">
                          {formatCur(result.costWithMarkup)}
                        </span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-slate-400">НДС 22%</span>
                        <span className="text-sm text-blue-400">
                          +{formatCur(result.vatAmount)}
                        </span>
                      </div>
                    </>
                  )}
