...
/**
 * Клиентская обёртка над списком контрагентов.
 *
 * Начальные данные (первая страница без фильтров) приходят со server-компонента
 * `page.tsx` через props — это исключает «спиннер на первый рендер» и сокращает
 * waterfall. Поиск/пагинация/CRUD остаются клиентскими и идут через /api/clients.
 */
export function ClientsView({
  ...
  const [clients, setClients] = useState<any[]>(initialItems);
  ...
  // Первый рендер использует initial-данные от сервера; перезапросы — только
  // когда меняется поиск/страница.
