...
// Error boundary для всего дашборда. Ловит любые серверные/клиентские
// исключения в сегменте и показывает аккуратный экран вместо дефолтного
// белого «Application error». reset() пере-рендерит сегмент, кнопка
// «Обновить» делает полную перезагрузку (помогает при устаревших server actions
// после деплоя).
    ...
    <div className="flex min-h-[60vh] items-center justify-center p-6">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
        ...
        <p className="mt-1.5 text-sm text-slate-500">
          ...
          помогает. Если повторяется, сообщите код ошибки ниже.
        </p>
        {error.digest && (
