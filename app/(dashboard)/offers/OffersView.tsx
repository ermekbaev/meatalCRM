...
/**
 * Клиентская обёртка над списком КП. Initial-данные приходят из server page.tsx,
 * фильтры/пагинация/CRUD остаются клиентскими (через /api/offers).
 */
export function OffersView({
  ...
  const [offers, setOffers] = useState<any[]>(initialItems);
  ...
  useEffect(() => { setPage(1); }, [search, statuses]);
  ...
  // Первый рендер использует initial-данные от сервера.
