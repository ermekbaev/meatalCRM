    ...
    <div>
      ...
      <div className="p-6 space-y-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button className="ml-auto" onClick={() => router.push("/offers/new")}>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={9} className="text-center py-8 text-gray-400">КП не найдены</TableCell></TableRow>
              ) : (
                offers.map((o) => (
                  <TableRow key={o.id}>
                    ...
                    <TableCell>
                      <div>
                        {o.request ? (
                          <p className="font-medium text-gray-900">{o.request.title}</p>
                        ) : <p className="text-gray-400">—</p>}
                        {o.request?.client && (
                          <p className="text-xs text-gray-500">{o.request.client.name}</p>
                        )}
                      </div>
                    </TableCell>
                    <TableCell>
                      <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${OFFER_STATUS_COLORS[o.status]}`}>
                        {OFFER_STATUS_LABELS[o.status]}
                      </span>
                    </TableCell>
                    <TableCell>{o.items?.length ?? 0}</TableCell>
                    <TableCell>{o.discount > 0 ? `${o.discount}%` : "—"}</TableCell>
                    <TableCell className="font-medium text-gray-900">{formatCurrency(o.total)}</TableCell>
                    <TableCell className="text-gray-500">{o.validUntil ? formatDate(o.validUntil) : "—"}</TableCell>
                    <TableCell className="text-gray-500">{formatDate(o.createdAt)}</TableCell>
                    <TableCell>
                      <div className="flex items-center gap-1">
                        <Button size="icon" variant="ghost" onClick={() => router.push(`/offers/${o.id}`)}>
