...
export default function RequestDetailPage() {
  ...
  const fetchFiles = useCallback(async () => {
    fetch(`/api/requests/${params.id}/files`)
      .then((r) => r.json())
      .then(setFiles)
      .catch(() => {});
  }, [params.id]);
  ...
  const deleteFile = async (fileId: string) => {
    await fetch(`/api/requests/${params.id}/files/${fileId}`, {
      method: "DELETE",
    });
  ...
  const updateItem = (idx: number, field: string, value: any) => {
    setItems((prev) => {
      ...
      const d =
        parseFloat(field === "discount" ? value : next[idx].discount) || 0;
              ...
              <CardHeader>
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-2 flex-wrap justify-end">
                    ...
                    <span
                      className={`rounded-full px-3 py-1 text-sm font-medium ${PRIORITY_COLORS[request.priority]}`}
                    >
                  ...
                  <>
                          ...
                          {items.map((item, index) => (
                            ...
                            >
                              ...
                              <td className="px-2 py-2">
                                {isEmployee ? (
                                  <span className="text-sm text-slate-800 block text-center">
                                    {item.quantity}
                                  </span>
                                ) : (
                                  <Input
                              ...
                              <td className="px-2 py-2">
                                {isEmployee ? (
                                  <span className="text-sm text-slate-800 block text-right">
                    ...
                    <div className="border-t border-slate-100 px-6 py-3 space-y-1.5">
                      <div className="flex justify-end gap-8 text-sm text-slate-500">
                        ...
                        <span>
                          {request?.vatIncluded ? "Без НДС:" : "Итого:"}
                        </span>
                        <span className="font-semibold text-slate-800 min-w-24 text-right">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      updateField("paymentMethod", v || null)
                    }
                    disabled={isEmployee}
                  >
                    <SelectTrigger>
                ...
                {subtotal > 0 && (
                  <div className="rounded-lg bg-slate-50 p-3 space-y-1.5">
                    <p className="text-xs text-slate-500">
                      {request?.vatIncluded ? "Без НДС" : "Сумма по позициям"}
                    </p>
                    ...
                    {request?.vatIncluded && vatAmount > 0 && (
                      <>
                        <p className="text-xs text-slate-400">
                          НДС 22%: +{formatCurrency(vatAmount)}
                        </p>
                  ...
                  <div className="divide-y divide-gray-100">
                    {request.offers.map((o: any) => (
                      ...
                      >
                        ...
                        <span
                          className={`rounded-md px-2 py-0.5 text-xs font-medium ${OFFER_STATUS_COLORS[o.status] ?? "bg-gray-100 text-gray-700"}`}
                        >
